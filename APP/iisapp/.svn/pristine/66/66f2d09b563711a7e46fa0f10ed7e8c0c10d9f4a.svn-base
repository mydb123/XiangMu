<template>
	<view>
		<view class="banner">
			...
			<image class="banner-img" :src="url"></image>
		</view>
...
<script>
	...
	export default {
		...
		methods:{
			getDetail(){
				let	carparkurl = urls.carpark_url+address.table
				get(carparkurl,this.queryData).then(res=>{
					this.url = urls.carpark_img_url+res[1].data.data[0].ImagePath
				})
			}
