<template>
  <div>
      <el-breadcrumb>
        ...
        <el-breadcrumb-item >原料库存</el-breadcrumb-item>
      </el-breadcrumb>
    <el-card>
      <el-row :gutter="20">
      ...
      <el-table :data="pageResult.rows" border>
        ...
        <el-table-column label="操作">
            <template slot-scope="scope">
                <el-button type="warning" @click="discard(scope.row.id)">废弃</el-button>
            </template>
        </el-table-column>
      </el-table>
      <!-- 分页 -->
...
export default {
  ...
  methods: {
    discard(id){
        this.$confirm('您确认废弃原料么?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(
          async () => {
            let msgRet = await api.raw.discard(id);
