...
export default {
    ...
    methods: {
        ...
        handleCurrentChange: function(currentPage){
            pageSearch(getPageDesignDept,pageRequest,this.pageResult,currentPage,this.formInline.name)
        },
        handleEdit(index,row){
           let router = getStatusDesignDeptRouter(row.status);
           this.$router.push(router + "?id=" + row.id);
        },
        onSubmit(){
            conditionSearch(getPageDesignDept,pageRequest,this.pageResult,this.formInline.name)
        },
        lookDetails(id){
    ...
    created() {
        createdSearch(getPageDesignDept,pageRequest,this.pageResult)
    },
}
</script>
 
