  ...
  <div id="informationChanges">
    ...
    <el-card>
      ...
      <el-table :data="pageResult.rows" border>
        ...
        <el-table-column prop="sourceArea" label="来源地区" width="160px">
...
import {pageRequest,pageResult,pageSearch,query,conditionSearch,createdSearch,getDonatorInformationList} from "@/utils/pageResult";
export default {
  data() {
    return {
      query:query,
  ...
  methods: {
    handleCurrentChange: function(val) {
      ...
      pageSearch(getDonatorInformationList,query,pageRequest,pageResult);
    },
    //条件查询
    onSubmit() {
      conditionSearch(getDonatorInformationList,query,pageRequest,pageResult);
    },
    //查看详情
    lookDetails(id) {
      this.$router.push({
  ...
  created() {
    createdSearch(getDonatorInformationList,query,pageRequest,pageResult);
  }
};
</script>
