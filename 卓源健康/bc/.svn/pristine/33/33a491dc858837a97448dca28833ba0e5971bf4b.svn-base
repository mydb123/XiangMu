<template>
  <div id="rawInspectionOperation" >
      <el-breadcrumb>
        <el-breadcrumb-item :to="{ path: '/workbench/raw/rawInspection'}">原料管理</el-breadcrumb-item>
        <el-breadcrumb-item :to="{ path: '/workbench/raw/rawInspection'}">原料初检</el-breadcrumb-item>
        <el-breadcrumb-item >录入原料初检信息</el-breadcrumb-item>
      </el-breadcrumb>
    <el-card>
    <el-form disabled>
        <div class="box" style="border:0px">
            <div class='content'>
              <el-row >
                  <el-col :span="4"><p>原料编码</p></el-col>
                  <el-col :span="8"><el-input class="w260" v-model="bizRaw.code"></el-input></el-col>
              </el-row>
            </div>
        </div>
        <div class="box" >
            <p class='clickbox' v-on:click="click(1)">原料来源信息</p>
            <div class='content' v-show="show1">
                <el-row >
                    <el-col :span="4"><p>材料名称</p></el-col>
                    <el-col :span="8">
                        <el-input class="w260" v-model="bizRaw.materialName" placeholder="请输入内容"></el-input>
                    </el-col>
                </el-row>
                <el-row >
                    <el-col :span="4"><p>采集人员</p></el-col>
                    <el-col :span="8">
                        <el-select class="w260" v-model="bizRaw.collectPerson" placeholder="请选择">
                            <el-option
                                v-for="item in collectors"
                                :key="item.id"
                                :label="item.user"
                                :value="item.id">
                            </el-option>
                        </el-select>
                    </el-col>
                    <el-col :span="4"><p>接收人员</p></el-col>
                    <el-col :span="8">
                        <el-select class="w260" v-model="bizRaw.receivePerson" placeholder="请选择">
                            <el-option
                                v-for="item in receptionStaff"
                                :key="item.id"
                                :label="item.user"
                                :value="item.id">
                            </el-option>
                        </el-select>
                    </el-col>
                </el-row>
                <el-row >
                        <el-col :span="4"><p>采集日期</p></el-col>
                        <el-col :span="8">
                            <el-date-picker
                                class="w260"
                                v-model="bizRaw.collectDate"
                                type="date"
                                value-format="yyyy-MM-dd">
                            </el-date-picker>
                        </el-col>
                </el-row>
            </div>
        </div>
    </el-form>
        <div class="box" >
            <p class='clickbox' v-on:click="click(2)">原料初检信息</p>
            <div class='content' v-show="show2">
                <el-form :model="formData" :rules="rules" ref="ruleForm">
                <el-row >
                    <el-col :span="4"><p class="fuji"><i class="ziji">*</i>初检评分</p></el-col>
                    <el-col :span="8">
                      <el-form-item prop="firstCheckScore">
                        <el-select class="w260" v-model="formData.firstCheckScore" placeholder="请选择">
                            <el-option
                                v-for="item in optionsScore"
                                :key="item.value"
                                :label="item.label"
                                :value="item.value">
                            </el-option>
                        </el-select>
                      </el-form-item>
                    </el-col>
                </el-row>
                <el-row>
                    <el-col :span="4"><p class="fuji"><i class="ziji">*</i>初检结果</p></el-col>
                    <el-col :span="8">
                      <el-form-item prop="firstCheckResult">
                        <el-select class="w260" v-model="formData.firstCheckResult" placeholder="请选择">
                            <el-option
                                v-for="item in optionsResult"
                                :key="item.value"
                                :label="item.label"
                                :value="item.value"
                                :disabled="item.disabled">
                            </el-option>
                        </el-select>
                      </el-form-item>   
                    </el-col>
                </el-row>
                <el-row >
                    <el-col :span="4"><p class="fuji"><i class="ziji">*</i>初检性状</p></el-col>
                    <el-col :span="8">
                      <el-form-item prop="firstCheckCharacter">
                        <el-input
                          type="textarea"
                          :autosize="{ minRows: 5, maxRows: 10}"
                          placeholder="请输入内容"
                          v-model="formData.firstCheckCharacter">
                        </el-input>
                      </el-form-item>  
                    </el-col>
                </el-row>
                </el-form>
            </div>
        </div>
        <div class='tac'>
            <el-button @click="save('ruleForm')" type="primary" round>提交</el-button>
        </div>
    </el-card>
  </div>
</template>

<script>
import api from "@/api/index";
import { FIRSTCHECK_SCORE,FIRSTCHECK_RESULT_COPY,STATION_TYPE } from "@/utils/util";

export default {
  data() {
    return {
      show1:true,
      show2:true,
      optionsScore:FIRSTCHECK_SCORE,
      optionsResult:FIRSTCHECK_RESULT_COPY,
      collectors:[],//采集人员列表
      receptionStaff:[],//接受人员列表
      bizRaw:{
        code:'',//原料编码
        materialName:'',//材料名称
        collectPerson:'',//采集人员
        receivePerson:'',//接收人员
        collectDate:'',//采集日期
      },
      formData:{
        id:'',
        firstCheckScore:'',//初检评分
        firstCheckCharacter:'',//初检性状
        firstCheckResult:''//初检结果
      },
      rules:{
        firstCheckScore: [
            { required: true, message: '请选择初检评分', trigger: 'change' },
        ],
        firstCheckCharacter: [
            { required: true, message: '请输入初检性状', trigger: 'blur' },
        ],
        firstCheckResult: [
            { required: true, message: '请选择初检结果', trigger: 'blur' },
        ]
      }
    };
  },
  methods: {
    click(index) {
      if (index == 1) {
        this.show1 = !this.show1;
      } else if (index == 2) {
        this.show2 = !this.show2;
      } 
    },
    //表单提交
    save(ruleForm){
      this.$refs[ruleForm].validate((valid) => {
        if (valid) {
          api.raw.firstCheckSave(this.formData).then(response =>{
          this.$router.push("/workbench/raw/rawInspection")
        }).catch(error=>{
          console.log(error)
        })
        } else {
          console.log('error submit!!');
          return false;
      }});
    },
    //初始化数据
    loadData(){
      //采集人员列表
      api.userRelated.getUserList(STATION_TYPE.COLLECT_PERSON).then(response =>{
        this.collectors = response;
      }),
      //接受人员列表
      api.userRelated.getUserList(STATION_TYPE.RECEIVE_PERSON).then(response =>{
        this.receptionStaff = response;
      })
      api.raw.findById(this.formData.id).then(response =>{
        this.bizRaw = response;
        //采集人员
        let collectPerson = response.collectPerson;
        this.bizRaw.collectPerson = collectPerson && collectPerson+'';
        //接受人员
        let receivePerson = response.receivePerson;
        this.bizRaw.receivePerson = receivePerson && receivePerson+'';
      }).catch((error)=>{
        console.log(error);
      });
    }
  },
  created(){
    this.formData.id = this.$route.query.id;
    this.loadData(); 
  }
}
</script>

<style lang="scss">
#rawInspectionOperation{
        .mb20{
            margin-bottom: 20px;
        }
        .tac{
            text-align: center;
            .el-button{
                width: 114px;
                height: 50px;
            } 
	    }
    }
</style>