<template>
    ...
    <div class='appraise_upload_purchase_partner'>
...
export default {
    ...
    methods: {
        submit() {
            api.purchasePartner.savePurchaseAppraise(this.formData).then(response => {
                this.$router.push('/purchase/partner/appraiseReview');
            }).catch(error => {
                console.log(error);
            });
        },
...
<style lang='scss'>
.appraise_upload_purchase_partner{
    margin: 10px auto;
