<template>
    ...
    <div class='plan_suggest_design_dept'>
...
export default {
  ...
  methods: {
    ...
    findBizDesignProgressByExample(id) {
      //加载项目最新的意见
      api.designDept.findBizDesignProgressByExample(id, 31)
        .then(response => {
  ...
  mounted: function() {
    ...
    this.findBizDesignProgressByExample(this.formData.id); //方案意见表信息
  }
};
</script>

<style lang='scss'>
.plan_suggest_design_dept {
  margin: 10px auto;
