<template>
    ...
    <div class='result_upload_construction_dept'>
...
export default {
    ...
    methods: {
        ...
        submit() {
            api.constructionDept.updateStatus(this.formData.id,51).then(response => {
                //console.log(response);      
                this.$router.push('/construction/dept/resultReview');
            }).catch(error => {
                console.log(error);
            });
        },
...
<style lang='scss'>
.result_upload_construction_dept{
    margin: 10px auto;
