<template>
    ...
    <div class='result_suggest_construction_dept'>
...
export default {
    ...
    methods: {
        ...
        submit() {
            api.constructionDept.updateStatus(this.formData.id,52).then(response => {
                //console.log(response);      
                this.$router.push('/construction/dept/resultUpdate');
            }).catch(error => {
                console.log(error);
            });            
        },
        ...
        loadResultFile(){//加载上传的成果附件
            api.constructionDept.loadFile(this.formData.id,5).then(response =>{
        ...
        findBizConstructionProgressByExample(){//加载项目信息和最新的意见
            api.constructionDept.findBizConstructionProgressByExample(this.formData.id,50).then(response => {
    ...
    mounted: function () {
        ...
        this.findBizConstructionProgressByExample();
    },
}
</script>

<style lang='scss'>
.result_suggest_construction_dept{
    margin: 10px auto;
