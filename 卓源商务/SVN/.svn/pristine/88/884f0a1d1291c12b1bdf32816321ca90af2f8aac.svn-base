<template>
<div class='all_purchase_partner'>
    <div class='title'>
        <el-button type="primary" @click="$router.push('/purchase/partner/approvalUpload')">
                创建项目
        </el-button>
    </div>
    <div class=''>
...
import {pageRequest,pageSearch,conditionSearch,createdSearch,getPagePurchasePartner} from '@/utils/pageResult';
export default {
    data() {
        return {
            show1:true,
    ...
    methods: {
        ...
        handleCurrentChange: function(currentPage){
            pageSearch(getPagePurchasePartner,pageRequest,this.pageResult,currentPage,this.formInline.name)
        },
        handleEdit(index,row){
            let r = getStatusPurchasePartnerRouter(row.status);
           this.$router.push({path:r,query:{id:row.id}});
        },
        onSubmit(){
            conditionSearch(getPagePurchasePartner,pageRequest,this.pageResult,this.formInline.name)
        },
        lookDetails(id){
            this.$router.push({path:"/purchase/partner/detail",query:{id:id}});
        }
    },
    created() {
        createdSearch(getPagePurchasePartner,pageRequest,this.pageResult)
    },
}
</script>
 
