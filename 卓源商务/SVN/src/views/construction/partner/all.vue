<template>
<div class='all_construction_partner'>
    <div class='title'>
        <el-button type="primary" @click="$router.push('/construction/partner/approvalUpload')">
            创建项目
        </el-button>
    </div>
    <div class=''>
...
import {pageRequest,pageSearch,conditionSearch,createdSearch,getPageConstructionPartner} from '@/utils/pageResult';
export default {
    data() {
        return {
            show1:true,
    ...
    methods: {
        ...
        handleCurrentChange: function(currentPage){
            pageSearch(getPageConstructionPartner,pageRequest,this.pageResult,currentPage,this.formInline.name)
        },
        handleEdit(index,row){
            let r = getStatusConstructionPartnerRouter(row.status);
            this.$router.push({path:r,query:{id:row.id}});
        },
        onSubmit(){
            conditionSearch(getPageConstructionPartner,pageRequest,this.pageResult,this.formInline.name)
        },
        lookDetails(id){
            this.$router.push({path:"/construction/partner/detail",query:{id:id}});
        }
    },
    created() {
        createdSearch(getPageConstructionPartner,pageRequest,this.pageResult)
    },
}
</script>
 
