<template>
    ...
    <div class='menu_review_construction_dept'>
        ...
        <div>
            ...
            <div class="wordborder" >
                ...
                <div class='content' v-show="show2">
                    ...
                    <el-row v-if="formData.otherRequire">
...
export default {
  ...
  methods: {
    ...
    download(){
        api.constructionDept.download(fileId).then(response => {
    ...
    submit() {
        api.constructionDept.updateStatus(this.formData.id,3).then(response => {
            this.$router.push({path:'/construction/dept/planUpload',query:{id:this.formData.id}});
        }).catch(error =>{
            console.log(error);
        });
    },
    ...
    loadFileElectronic(){//电子版图纸附件
        api.constructionDept.loadFile(this.formData.id,0).then(response =>{
    ...
    loadFileIntelligent(){//智能化图纸附近
        api.constructionDept.loadFile(this.formData.id,1).then(response =>{
    ...
    loadFileDecoration(){//装饰化图纸
        api.constructionDept.loadFile(this.formData.id,2).then(response =>{
    ...
    loadFileList(){//采购清单
        api.constructionDept.loadFile(this.formData.id,3).then(response =>{
