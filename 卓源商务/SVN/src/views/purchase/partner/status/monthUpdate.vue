<template>
    ...
    <div class='month_update_purchase_partner'>
    ...
    methods: {
        ...
        loadMonthData(id){
            api.purchasePartner.findMouthListByExample(this.formData.id).then(response => {
                ...
                this.findBizPurchaseProgressByExample(this.monthId);//加载本月度最新意见
            }).catch(error => {
                console.log(error);
            });
        },
        findBizPurchaseProgressByExample(monthId){
            api.purchasePartner.findBizPurchaseProgressByExampleMonth(this.formData.id,monthId,40).then(response => {
