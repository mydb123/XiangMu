...
import {pageRequest,pageSearch,conditionSearch,createdSearch,getPageDesignDept} from '@/utils/pageResult';
export default {
    data() {
        return {
            show1:true,
    ...
    methods: {
        ...
        handleCurrentChange: function(currentPage){
            pageSearch(getPageDesignDept,pageRequest,this.pageResult,currentPage,this.formInline.name,7)
        },
        handleEdit(index,row){
            let r = getStatusDesignDeptRouter(row.status);
            this.$router.push({path:r,query:{id:row.id}});
        },
        onSubmit() {
            conditionSearch(getPageDesignDept,pageRequest,this.pageResult,this.formInline.name,7)
        },
        lookDetails(id){
            this.$router.push({path:"/design/dept/detail",query:{id:id}});
        }
    },
    created() {
        createdSearch(getPageDesignDept,pageRequest,this.pageResult,7)
    },
}
</script>
 
