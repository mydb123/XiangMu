<template>
    ...
    <div class='result_review_design_partner'>
...
export default {
    ...
    data() {
        return {
            downloadUrl:base.sq+"/bizDesign/partner/download?access_token="+store.getters.token+"&fileId=",//下载路径 
            show1:true,
            show2:true,
            index:5,
    ...
    methods: {
                ...
                if(valid){
                    api.designPartner.updateStatusAndProgress(this.formData.id,51,this.formData.progress).then(response => {
                        if(!response){
                            ...
                            this.$router.push(getStatusDesignPartnerRouter(51));
                        }else{
        ...
        accept() {
            api.designPartner.updateStatus(this.formData.id,6).then(response => {
                this.$message.success("提交成功");
                this.$router.push(getStatusDesignPartnerRouter(6)+'?id='+this.formData.id);
            }).catch(error => {
                console.log(error);
            });
        },
        ...
        loadResultFile(){//加载上传的成果附件
            api.designPartner.loadFile(this.formData.id,4).then(response =>{
