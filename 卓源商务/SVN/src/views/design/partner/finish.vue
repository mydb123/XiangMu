...
<div class='finish_design_partner'>
...
export default {
    ...
    methods: {
        ...
        handleCurrentChange: function(currentPage){
            pageSearch(getPageDesignPartner,pageRequest,this.pageResult,currentPage,this.formInline.name,7)
        },
        handleEdit(index,row){
            let r = getStatusDesignPartnerRouter(row.status);
            this.$router.push({path:r,query:{id:row.id}});
        },
        onSubmit() {
            conditionSearch(getPageDesignPartner,pageRequest,this.pageResult,this.formInline.name,7)
        },
        lookDetails(id){
    ...
    created() {
        createdSearch(getPageDesignPartner,pageRequest,this.pageResult,7)
    },
}
</script>
 
