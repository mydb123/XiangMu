<template>
    ...
    <div class='result_update_construction_partner'>
                    ...
                    <el-row >
                        ...
                        <el-col :span="19">
                            <div v-for="item in resultFile" :key="item.id" class="text item">
                                <el-link type="primary" :href="downloadUrl+item.id">{{item.showName}}</el-link>
                            </div>   
                        </el-col>
                    </el-row>
                </div>
...
export default {
    ...
    methods: {
        ...
        refuse() {
            api.constructionPartner.updateStatusAndProgress(this.formData.id,50,this.progress).then(response => {
                this.$router.push({path:'/construction/partner/resultSuggest',query:{id:this.formData.id}});
            }).catch(error => {
                console.log(error);
            });
        },
        accept() {
            api.constructionPartner.updateStatus(this.formData.id,6).then(response => {
                ...
                // this.$router.push('/construction/partner/appraiseUpload');
        ...
        findBizConstructionProgressByExample(){//加载项目信息和最新的意见
            api.constructionPartner.findBizConstructionProgressByExample(this.formData.id,50).then(response => {
    ...
    mounted: function () {
        ...
        this.loadTargetFile();//加载经营目标预算附近
        this.loadResultFile();//加载成果附件    
        this.findBizConstructionProgressByExample();//最新意见  
    }
}
</script>

<style lang='scss'>
.result_update_construction_partner{
    margin: 10px auto;
