<template>
    ...
    <div class='plan_suggest_purchase_dept'>
...
export default {
    ...
    data() {
        return {
            ...
            rules:{
                ...
                buyerPhone: [
                    { required: true, message: '请输入采购人联系方式', trigger: 'blur' }
                ],
                procurementScope: [
    ...
    methods: {
            ...
            this.$refs[ruleForm].validate((valid) => {
                if (valid) {
                    api.purchaseDept.savePurchasePlan(this.formData).then(response => {
                        this.$router.push('/purchase/dept/planUpdate');
                    }).catch(error => {
                        console.log(error);
                    });         
                } else {
        ...
        findBizPurchaseProgressByExample(id){//加载项目信息和最新的意见
            api.purchaseDept.findBizPurchaseProgressByExample(id,30).then(response => {
    ...
    mounted: function () {
        ...
        this.findBizPurchaseProgressByExample(this.formData.id);//意见
