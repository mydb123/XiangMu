<template>
    ...
    <div class='appraise_review_purchase_dept'>
...
export default {
    ...
    methods: {
        submit() {
            api.purchaseDept.updateStatus(this.formData.id,7).then(response => {
                this.$router.push('/purchase/dept/finishReview');
            }).catch(error => {
                console.log(error);
            });
        },
